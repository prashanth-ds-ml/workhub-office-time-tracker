...
import {
  Activity, BarChart3, Bell, Building2, BriefcaseBusiness, CalendarDays, ChevronLeft,
  ChevronRight, ChevronsLeft, ChevronsRight, CircleStop, Clock3, Coffee,
  Download, Eye, EyeOff, Gauge, History, Home, LogOut, Menu, Megaphone, Play, Plus,
...
export default function App() {
  ...
  const [auth, setAuth] = useState(saved), [page, setPage] = useState(saved?.user?.role === "Boss" ? "employees" : "dashboard"), [month, setMonth] = useState(currentMonth());
  ...
  const isBoss = auth?.user?.role === "Boss";
  const BOSS_PAGES = new Set(["employees", "reports", "attendance"]);
  ...
  const acceptAuth = value => {
    ...
    setData(emptyWorkspace);
    setPage(nextAuth.user?.role === "Boss" ? "employees" : "dashboard");
  ...
  const action = async (kind, workLocation) => {
    ...
    const body = kind === "start" ? { work_location: workLocation || "office" } : undefined;
    setBusy(true);
    try {
      const result = await api(path,{token:auth.token,method:"POST",body});
  ...
  return <div className="app-shell">
    <aside className={mobile ? "open" : ""}><div className="logo"><img className="launcher-mark sidebar-launcher" src="/med360-launcher.svg" alt="Med 360+" /><div><strong>WorkHub</strong><span>Med 360+ workspace</span></div><button className="icon-button close-nav" onClick={()=>setMobile(false)}><X/></button></div>
      <nav>{nav.filter(x=>isBoss?BOSS_PAGES.has(x[0]):(!x[3]||(x[3]==="manage"?canManage:admin))).map(([key,label,Icon])=><button key={key} className={page===key?"active":""} onClick={()=>{setPage(key);setMobile(false)}}><Icon/>{label}{key==="announcements"&&data.overview.unread_announcements>0&&<i>{data.overview.unread_announcements}</i>}</button>)}</nav>
    ...
    <main><header className="topbar"><button className="icon-button menu" onClick={()=>setMobile(true)}><Menu/></button><div><h1>{title}</h1><p>{canManage ? "Manager console" : admin ? "Boss console" : "Employee workspace"}</p></div>
      ...
      <div className="content">{error&&<div className="banner error">{error}<button onClick={()=>setError("")}><X/></button></div>}
        ...
        {page==="dashboard"&&!isBoss&&<Dashboard overview={data.overview} serverOffsetMs={serverOffsetMs}/>}
        {page==="calendar"&&!isBoss&&<Calendar events={data.overview.calendar_month||[]} month={month} setMonth={setMonth} admin={canManage} onAdd={()=>setModal("event")}/>}
