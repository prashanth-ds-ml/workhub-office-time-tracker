...
export default function App() {
  ...
  const loadSection = useCallback(async section => {
    ...
    try {
      ...
      if (section === "attendance") {
        ...
        const analytics = await api(`/admin/analytics?month=${requestedMonth}`, { token: auth.token });
        if (latestMonthRef.current !== requestedMonth) return;
        setData(previous => {
  ...
  }, [auth, hasOverview, month, admin, logout, persistWorkspace]);
  ...
  const exportAnalytics = () => {
    ...
    downloadCsv(`workhub-analytics-${month}.csv`, rows);
  };
